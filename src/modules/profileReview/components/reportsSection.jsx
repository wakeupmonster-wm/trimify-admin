...
const ReportItem = ({ r }) => {
  ...
  return (
    <Card className="group rounded-xl border-slate-200 shadow-sm bg-white overflow-hidden mb-3 last:mb-0 transition-all shrink-0">
      <CardContent className="p-0">
        {/* 1. Header Section */}
        <div className="flex items-center justify-between px-8 pb-4 bg-slate-50/30 border-b border-slate-200">
        ...
        <div className="px-8 pt-5 space-y-4">
          ...
          {/* 3. Description Box */}
          <div className="bg-slate-50 border border-slate-300/80 rounded-lg px-6 py-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {r.replyHistory && r.replyHistory.length > 0 && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-white border border-slate-200 text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition-all shadow-sm"
                    >
                ...
                {r.replyHistory && r.replyHistory.length > 0 && showHistory && (
                  <div className="mt-2 space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                    ...
                    {r.replyHistory.map((reply, hIdx) => (
                      <div
                        key={hIdx}
                        className="flex flex-col gap-1.5 bg-white/60 p-3.5 rounded-xl border border-slate-200 shadow-sm"
                      >
                        <div className="flex items-center justify-between">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={cn(
        ...
        reportsToDisplay.length > 0 && "max-h-[600px] overflow-y-auto pr-4 custom-scrollbar"
      )}>
        {reportsToDisplay.length === 0 ? (
