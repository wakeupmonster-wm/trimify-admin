...
const ManageBlogsPage = () => {
    ...
    const { 
      posts, postsLoading, postsPagination
    } = useSelector((state) => state.blogSection);

    const [postFilter, setPostFilter] = useState("");
    const [postPage, setPostPageState] = useState({ pageIndex: Math.max(0, postsPagination.page - 1), pageSize: postsPagination.limit || 10 });
      ...
      <Container>
        ...
        <div className='space-y-8'>
          ...
          <DataTable
            columns={postColumns}
            data={displayPosts}
            rowCount={isPostManual ? postsPagination.total : (displayPosts?.length || 0)}
            pagination={postPage}
