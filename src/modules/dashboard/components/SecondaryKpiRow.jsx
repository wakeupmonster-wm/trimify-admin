  ...
  return (
    <div
      className={`flex items-center gap-1 font-bold text-[10px] border rounded-full py-1 px-2 shrink-0 ${
        isUp ? "text-emerald-600 bg-emerald-50 border-emerald-200" : "text-rose-600 bg-rose-50 border-rose-200"
      }`}
    >
      {isUp ? <IconTrendingUp size={12} stroke={3} /> : <IconTrendingDown size={12} stroke={3} />}
      {change}
    </div>
  );
};
...
const SecondaryKpiRow = ({ data, selectedDate }) => {
  ...
  const goTo = (route) => () =>
    navigate(route, { state: { preset: selectedDate?.preset, from: selectedDate?.from, to: selectedDate?.to } });

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
      <Card>
        <div className="flex items-start justify-between gap-2">
          <p className="text-xs font-semibold text-foreground/70">New Signups</p>
        ...
        <div>
          ...
          <div className="flex items-center gap-3 text-[11px] font-semibold text-slate-500">
            <span>Today <b className="text-slate-800">{data.newSignups.today}</b></span>
            <span className="w-1 h-1 rounded-full bg-slate-300" />
            <span>Week <b className="text-slate-800">{data.newSignups.week}</b></span>
          </div>
        </div>
      </Card>
      ...
      <Card>
        <div className="flex items-start justify-between gap-2">
          <p className="text-xs font-semibold text-foreground/70">Active Subscriptions</p>
      ...
      <Card tone="warning" onClick={goTo("/admin/subscription-management/subscribers")}>
        <div className="flex items-start justify-between gap-2">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-medium mt-1">Next 7 days · tap to view</p>
        </div>
      </Card>

      <Card tone="danger" onClick={goTo("/admin/subscription-management/subscribers")}>
        <div className="flex items-start justify-between gap-2">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-medium mt-1">{data.churn.rate} churn rate · tap to view</p>
        </div>
      </Card>

      <Card tone="danger" onClick={goTo("/admin/transaction-management")}>
        <div className="flex items-start justify-between gap-2">
          <p className="text-xs font-semibold text-rose-700">Failed Transactions</p>
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-medium mt-1">tap to view list</p>
        </div>
      </Card>
    </div>
  );
