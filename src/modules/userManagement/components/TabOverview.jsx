    ...
    <>
      ...
      <div className="flex flex-col gap-3.5">
        ...
        >
          ...
          />
          {user.sub_admin ? (
            <KV icon={User} label="Managed By" value={user.sub_admin?.name} noBorder={true} />
          ) : (
            <KV
              icon={User}
              label="Managed By"
        ...
        <Card title="Location & Context" subtitle="User geographic summary">
          <KV
            icon={MapPin}
            label="City/State"
            value={user.city || user.state ? `${user.city || ''}${user.city && user.state ? ', ' : ''}${user.state || ''}` : "Not Provided"}
            noBorder={true}
          />
          <KV
            icon={Target}
        ...
        <Card title="Account Reports" subtitle="User standing & flags">
          <div className="p-4 rounded-lg bg-green-50/50 flex items-center gap-4 border border-green-100">
            ...
            <div>
              ...
              <p className="text-[10px] text-green-600 font-medium mt-0.5">No reports filed against this user</p>
            </div>
          </div>
        </Card>
      </div>
