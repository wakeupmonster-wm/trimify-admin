            ...
            {visibleActivities.map((item) => {
              ...
              return (
                <div key={item.id} className="flex items-start gap-4">
                  <div
                    className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 shadow-sm ring-4 ring-slate-50 ${item.bg || "bg-slate-100"} ${item.color || "text-slate-600"}`}
                  >
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <div className="text-[9px] font-bold text-slate-400 uppercase tracking-wider shrink-0 mt-0.5">
                        {item.time}
                      </div>
                    </div>
                    <div className="text-[12px] font-medium text-slate-500 mt-1 leading-relaxed">
                      {item.desc}
                    </div>
                  </div>
                </div>
