...
const ProfileHeader = ({ userData, isActive, formatDate }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-3xl shadow-sm border border-slate-300/60/60 hover:border-blue-200 p-6 md:p-8 mb-6 relative overflow-hidden flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
      {/* Decorative Blob */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-app-primary2/5 rounded-full blur-3xl -z-10 pointer-events-none translate-x-1/3 -translate-y-1/3"></div>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-[13px] font-medium text-slate-500">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-slate-700 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-300/60 select-all">
                {userData.user_id || "-"}
              </span>
            </div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
        <Dialog open={emailOpen} onOpenChange={setEmailOpen}>
          <DialogTrigger asChild>
            <Button
              variant="outline"
              className="w-full sm:w-auto h-10 px-5 border-app-primary2/20 text-app-primary2 hover:!bg-app-primary2 hover:!border-app-primary2/40 hover:!text-white font-semibold rounded-xl shadow-sm gap-2 transition-all"
            >
        ...
        <Dialog open={pushOpen} onOpenChange={setPushOpen}>
          <DialogTrigger asChild>
            <Button className="w-full sm:w-auto h-10 px-5 bg-app-primary2 hover:bg-slate-50 border border-blue-50 hover:border hover:border-app-primary2 hover:text-app-primary2 text-white font-semibold rounded-xl shadow-sm gap-2 transition-all">
