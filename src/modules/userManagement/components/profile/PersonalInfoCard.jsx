  ...
  return (
    <SectionCard title="Personal Information" subheading="Basic details and biometrics" icon={LuUserRound}>
       <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-8 gap-x-6">
          ...
          <GridItem label="Height" value={userData.height ? `${userData.height} ${userData.height_unit || "cm"}` : "-"} icon={Ruler} iconColor="text-slate-400" />
          <GridItem label="Weight" value={userData.weight ? `${userData.weight} ${userData.weight_unit || "kg"}` : "-"} icon={Scale} iconColor="text-slate-400" />
