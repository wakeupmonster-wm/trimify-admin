...
export function DataTablePagination({ table, rowCount, itemName = "items" }) {
  ...
  const showingText = (
    <div className="text-xs font-medium text-slate-400 text-left whitespace-nowrap">
  ...
  return (
    <div className="border-t border-slate-300/60 p-4 sm:p-6">
      {/* Mobile & tablet: "Showing" left / "Rows" right on one row, page
          numbers on their own row below — avoids cramming everything into
          a single row on narrow/tablet widths. */}
      <div className="flex flex-col gap-4 lg:hidden">
        <div className="flex items-center justify-between gap-4 w-full">
          {showingText}
          {rowsSelector}
        </div>
        <div className="w-full flex justify-center">{pageButtons}</div>
      </div>
