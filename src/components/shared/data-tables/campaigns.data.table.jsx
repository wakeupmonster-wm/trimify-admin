                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <span className="text-xs">
                      {filters.drawStatus
                        ? filters.drawStatus.charAt(0).toUpperCase() +
                          filters.drawStatus.slice(1).toLowerCase()
                        : "All Status"}
                    </span>
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <TableRow key={headerGroup.id}>
                  {headerGroup.headers.map((header) => (
                    <TableHead
                      key={header.id}
                      className="text-foreground/80 px-3 md:px-6 font-bold uppercase h-10 bg-slate-100/50 text-[10px] text-left"
                    >
                      {flexRender(
                  ...
                  table.getRowModel().rows.map((row, index) => (
                    <motion.tr
                      ...
                      onClick={(e) => {
                        if (
                          ...
                          return;
                        navigate(
                          `/admin/management/giveaway/view-campaign/${row.original?._id}`,
                          {
                            state: {
                    ...
                    >
                      {row.getVisibleCells().map((cell) => (
                        <TableCell
                          key={cell.id}
                          className="py-3 px-3 md:px-6 text-left"
                        >
                          {flexRender(
