...
export default function FakeProfileDataTable({
  ...
  const hasActiveFilters = !!filters.statusFilter || !!filters.genderFilter || !!filters.planFilter;
  ...
  const handleGenderChange = (gender) => {
    filters.setGenderFilter(filters.genderFilter === gender ? "" : gender);
    onPaginationChange((p) => ({ ...p, pageIndex: 0 }));
  };

  const handlePlanChange = (plan) => {
    filters.setPlanFilter(filters.planFilter === plan ? "" : plan);
    onPaginationChange((p) => ({ ...p, pageIndex: 0 }));
  };
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      "h-9 3xl:h-10 px-3 3xl:px-4 gap-2 bg-white border-slate-200 text-slate-600 font-medium rounded-md hover:bg-slate-50 transition-all w-full md:w-auto justify-between",
                      filters.genderFilter && "border-brand-aqua text-brand-aqua",
                    )}
                  >
                    <span className="text-xs">
                      {filters.genderFilter
                        ? filters.genderFilter.charAt(0).toUpperCase() +
                        filters.genderFilter.slice(1)
                        : "All Genders"}
                    </span>
                    <IconChevronDown className="h-4 w-4 opacity-50" />
                  </Button>
                ...
                >
                  ...
                  {GENDER_OPTIONS.map((g) => (
                    <DropdownMenuCheckboxItem
                      key={g}
                      className="rounded-lg capitalize text-xs"
                      checked={filters.genderFilter === g}
                      onCheckedChange={() => handleGenderChange(g)}
                    >
                      {g.replace("-", " ")}
                    </DropdownMenuCheckboxItem>
                  ))}
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      "h-9 3xl:h-10 px-3 3xl:px-4 gap-2 bg-white border-slate-200 text-slate-600 font-medium rounded-md hover:bg-slate-50 transition-all w-full md:w-auto justify-between",
                      filters.planFilter && "border-brand-aqua text-brand-aqua",
                    )}
                  >
                    <span className="text-xs">
                      {filters.planFilter === "premium"
                        ? "Premium"
                        : filters.planFilter === "free"
                          ? "Free"
                          : "All Plans"}
                    </span>
                    <IconChevronDown className="h-4 w-4 opacity-50" />
                ...
                >
                  ...
                  <DropdownMenuCheckboxItem
                    ...
                    checked={filters.planFilter === "premium"}
                    onCheckedChange={() => handlePlanChange("premium")}
                  >
                    Premium Only
                  </DropdownMenuCheckboxItem>
                  <DropdownMenuCheckboxItem
                    className="rounded-lg text-xs"
                    checked={filters.planFilter === "free"}
                    onCheckedChange={() => handlePlanChange("free")}
                  >
                    Free Only
                  </DropdownMenuCheckboxItem>
                </DropdownMenuContent>
              ...
              >
                {filters.statusFilter && (
                  ...
                  >
                    ...
                    <button
                      onClick={() => handleStatusChange(filters.statusFilter)}
                ...
                {filters.genderFilter && (
                  ...
                  >
                    ...
                    <button
                      onClick={() => handleGenderChange(filters.genderFilter)}
                      className="hover:bg-slate-200 rounded-full p-0.5 transition-colors"
                    >
                      <IconX size={10} />
                    </button>
                ...
                {filters.planFilter && (
                  ...
                  >
                    ...
                    <button
                      onClick={() => handlePlanChange(filters.planFilter)}
                      className="hover:bg-slate-200 rounded-full p-0.5 transition-colors"
                    >
                      <IconX size={10} />
                    </button>
      ...
      <div className="relative rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
              ...
              <AnimatePresence mode="popLayout">
                {table.getRowModel().rows?.length ? (
                  ...
                  <TableRow>
                    ...
                    >
                      {isLoading ? (
                        <TableLoader text="Fetching Fake Profiles..." />
                      ) : (
        ...
        <div className="flex flex-col items-start justify-between p-4 sm:p-6 border-t border-slate-200 gap-6 sm:flex-row sm:gap-4">
          ...
          <div className="text-xs font-medium text-slate-400 order-1 text-center sm:text-left">
            ...
            of {rowCount} profiles
          </div>

          {/* Right Side: Pagination Controls */}
          <div className="flex items-center gap-6 order-2 w-full sm:w-auto">
            {/* Row Select Bar */}
